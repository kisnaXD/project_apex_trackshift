...
class LiveTelemetryBridge:
    ...
    def assemble_and_publish(self, ego_model, ego_s, ego_y, tire_model, opponents_dict, drs_active, sim_time=0.0):
        ...
        tires = TireChannel(
            ...
            surface_temp_c={
                "FL": float(tire_model.T_surf), "FR": float(tire_model.T_surf),
                "RL": float(tire_model.T_surf), "RR": float(tire_model.T_surf)
            },
            carcass_temp_c={
                "FL": float(tire_model.T_carc), "FR": float(tire_model.T_carc),
                "RL": float(tire_model.T_carc), "RR": float(tire_model.T_carc)
            }
        )

        # 4. Assembled Ego Telemetry
